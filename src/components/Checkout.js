...
const Checkout = () => {
  ...
  const addAddress = async (token, newAddress) => {
    try {
      ...
      axios
        .post(`${config.endpoint}/user/addresses`, obj,
          {
            headers: {
            "Authorization": `Bearer ${token}`
            }   
  ...
  const performCheckout = async (token, items, addresses) => {
    if(!validateRequest(items, addresses)){
      ...
      try{
        axios.post(`${config.endpoint}/cart/checkout`, {"addressId":addresses.selected},
          {
            headers: {
            "Authorization": `Bearer ${token}`
            }   
          ...
          let val = localStorage.getItem("balance") - getTotalCartValue(items)
  ...
  function resetAddressSelection(){
    try{
      if(addresses.all){
        addresses.all.forEach(x => {
          if(document.getElementById(x._id).classList.contains("selected")){
            document.getElementById(x._id).classList.remove("selected");
            document.getElementById(x._id).classList.add("not-selected");
          }
        })
      }
    }catch(e){
  ...
  const handleAddrClick = (event) => {
    resetAddressSelection()
    event.target.classList.add("selected");
    setAddresses({ ...addresses, selected: event.target.id });
  }
  return (
    <>
      <Header />
